...
exports.register = asyncHandler(async (req, res, next) => {
	...
	try {
		if(full_name === undefined || username === undefined || email === undefined || password === undefined){
		...
		if (userExists) {
			return res.status(500).json({
				status: false,
				message: `${userExists.email} is already registered`,
			});
		}
...
exports.login = asyncHandler(async (req, res, next) => {
	...
	// Check for user
	const user = await User.findOne({ username: email }).select('+password') || await User.findOne({ email: email }).select('+password');
	if (!user) {
